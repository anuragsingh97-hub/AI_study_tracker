...
export default function CameraPreview({
  ...
  return (
    <motion.div
      ...
      className="bg-slate-900 rounded-3xl border border-slate-700 overflow-hidden shadow-xl"style={{padding:"10px",margin:"3px"}}
    >
      {/* Header */}
      <div className="flex justify-between items-center p-5 border-b border-slate-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">{active ? "Live Monitoring" : "Monitoring paused"}</p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        /> : <div className="flex h-[350px] items-center justify-center bg-slate-950 px-6 text-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">Start or resume your study session to enable AI monitoring.</p>
          </div>
        </div>}
        {active && <DetectionOverlay
          detections={detections}
          videoWidth={1280}
          videoHeight={720}
        />}
        {active && <div className="absolute top-4 left-4 bg-black/60 backdrop-blur px-3 py-2 rounded-xl flex gap-2">
          <ScanFace className="text-blue-400" size={18} />
          <span className="text-white text-sm">AI Vision</span>
        </div>}
        {active && <div className="absolute bottom-4 left-4 bg-black/60 px-3 py-2 rounded-xl">
          <p className="text-white text-sm">Faces Detected: {faceCount}</p>
        </div>}
        {active && <button type="button" aria-label="Expand camera preview" className="absolute top-4 right-4 bg-black/60 p-2 rounded-xl">
