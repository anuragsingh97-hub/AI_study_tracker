...
export default function StudyNotes({ notes, setNotes }) {
  return (
    <motion.div
      ...
      className="bg-slate-900 border border-slate-700 rounded-3xl p-6 shadow-xl"style={{padding:"10px",margin:"3px"}}
    >
      ...
      <div className="flex gap-4 mt-5">
        <button type="button" className="flex-1 flex justify-center items-center gap-2 bg-blue-600 hover:bg-blue-700 rounded-xl py-3 text-white">
          <Save size={18} />
          Save
        </button>
        <button type="button" onClick={() => setNotes("")} aria-label="Clear notes" className="bg-red-600 hover:bg-red-700 rounded-xl px-5 text-white">
          <Trash2 size={18} />
        </button>
      </div>
    </motion.div>
